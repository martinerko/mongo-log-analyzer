...
function analyzeQueryText(queryText) {
  try {
    var q = JSON.parse(queryText, queryReviver);
    var orderText = queryText.match(REGEXP_PARSE_ORDERBY_INFO);
    var info = {
      $query: stringifyCanonical(q.hasOwnProperty('$query') ? q['$query'] : q),
      $orderby: orderText ? orderText[1] : '{}'
    };
    return info;
  } catch (e) {
